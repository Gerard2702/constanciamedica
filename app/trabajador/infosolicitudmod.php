...
<?php   		
   		...
   		$sqlcomprobar = "SELECT id_datos FROM datos_iniciales WHERE id_datos=? AND id_estado=5 AND id_trabajador=?";
      ...
      if($stmtcomen = $conn->prepare($sqlcomentarios)){
        $stmtcomen -> bind_param('i',$contancianum);
        $stmtcomen -> execute();
        $stmtcomen -> store_result();
        $rowscomen = $stmtcomen->num_rows;
        $stmtcomen -> bind_result($comentarios);
      }
